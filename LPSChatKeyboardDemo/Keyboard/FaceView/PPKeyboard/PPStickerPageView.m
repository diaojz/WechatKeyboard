...
NSUInteger const PPStickerPageViewMaxEmojiCount = 23;
static NSUInteger const PPStickerPageViewLineCount = 3;
static NSUInteger const PPStickerPageViewButtonPerLine = 8;
...
- (UIButton *)deleteButton
{
    if (!_deleteButton) {
        ...
        [_deleteButton setImage:[UIImage imageWithBundleName:@"delete-emoji"] forState:UIControlStateNormal];
...
- (void)configureWithSticker:(PPSticker *)sticker
{
    ...
    for (PPEmoji *emoji in emojis) {
        ...
        [button setBackgroundImage:[self emojiImageWithName:emoji.imageName] forState:UIControlStateNormal];
        index += 1;
    }
...
- (UIImage *)emojiImageWithName:(NSString *)name
{
    ...
    UIImage *image = [UIImage imageWithFaceBundleName:name];
    return image;
}
