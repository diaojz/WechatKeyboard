...
static CGFloat const PPPreviewViewWidth = 90.0;
static CGFloat const PPPreviewViewHeight = 136.0;
...
- (void)layoutSubviews
{
    ...
    self.topLine.frame = CGRectMake(0, 0, CGRectGetWidth(self.bounds), 1 / [UIScreen mainScreen].scale);
...
- (UIView *)topLine
{
    if (!_topLine) {
        ...
        _topLine.backgroundColor = [UIColor rr_colorWithHexValue:0xd8d8d9];
    }
...
- (UIImage *)emojiImageWithName:(NSString *)name
{
    ...
    return [UIImage imageWithFaceBundleName:name];
}
...
- (void)stickerPageView:(PPStickerPageView *)stickerKeyboard showEmojiPreviewViewWithEmoji:(PPEmoji *)emoji buttonFrame:(CGRect)buttonFrame
{
    ...
    self.emojiPreviewView.frame = CGRectMake(CGRectGetMidX(buttonFrameAtKeybord) - PPPreviewViewWidth / 2, UIScreen.mainScreen.bounds.size.height - CGRectGetHeight(self.bounds) + CGRectGetMaxY(buttonFrameAtKeybord) - CGRectGetHeight(buttonFrameAtKeybord) - PPPreviewViewHeight, PPPreviewViewWidth, PPPreviewViewHeight);
