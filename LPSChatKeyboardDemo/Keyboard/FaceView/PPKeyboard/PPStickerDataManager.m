...
- (NSAttributedString *)replaceEmojiForString:(NSString *)string attributes:(nullable NSDictionary<NSAttributedStringKey, id> *)attrs
{
    ...
    if (results && results.count) {
        ...
        for (PPStickerMatchingResult *result in results) {
            ...
            imageView.size = CGSizeMake(font.lineHeight+3, font.lineHeight+3);
            NSMutableAttributedString *emojiAttributedString = [NSMutableAttributedString yy_attachmentStringWithContent:imageView contentMode:UIViewContentModeCenter attachmentSize:imageView.size alignToFont:font alignment:YYTextVerticalAlignmentCenter];
